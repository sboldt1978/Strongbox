...
class SwiftUIViewFactory: NSObject {
#if !IS_APP_EXTENSION
    @objc
    static func showKeyFileGeneratorScreen(
        keyFile: KeyFile,
    ...
    @objc
    static func showKeyFileRecoveryScreen(
        _ onRecover: @escaping ((_ keyFile: KeyFile) -> Void)
    ) {
        let window = EscapableWindow(
            contentRect: NSRect(x: 0, y: 0, width: 480, height: 800),
            styleMask: [.titled, .closable],
    ...
    @objc
    static func makeImportResultViewController(
        messages: [ImportMessage] = [],
        dismissHandler: @escaping ((_ cancel: Bool) -> Void)
    ) -> NSViewController {
    ...
    @objc
    static func makeWiFiSyncPassCodeEntryViewController(
        _ server: WiFiSyncServerConfig,
        onDone: @escaping ((_ server: WiFiSyncServerConfig?, _ pinCode: String?) -> Void)
    ) -> NSViewController {
    ...
    @objc
    static func makeSaleOfferViewController(
        sale: Sale,
    ...
    @objc
    static func makeStorageSelector(
        createMode: Bool,
        isImporting: Bool,
        isPro: Bool,
        cloudKitUnavailableReason: String?,
        initialSelection: StorageProvider,
        completion: @escaping ((_ userCancelled: Bool, _ selected: StorageProvider, _ selectedWiFiSyncDevice: WiFiSyncServerConfig?) -> Void)
    ) -> NSViewController {
    ...
    @objc
    static func getVirtualHardwareKeyCreateView(
        completion: @escaping ((_ userCancelled: Bool, _ name: String, _ secret: String, _ fixedLength: Bool) -> Void)
    ) -> NSViewController {
        NSHostingController(rootView: CreateVirtualHardwareKeyView(completion: completion))
    }
    
    @objc
    static func getHardwareKeySettingsView(
        metadata: METADATA_PTR,
        onSettingsChanged: ((Bool, Int, Int, Bool) -> Void)?,
    ...
    @objc
    static func getCreateOrAdd2FACodeView(
        token: OTPToken,
        model: Model,
        completion: @escaping ((_ cancel: Bool, _ createNew: Bool, _ title: String?, _ group: Node?, _ selectedEntry: UUID?) -> Void)
    ) -> NSViewController {
    ...
    @objc
    static func getDuplicateItemOptionsView(
        showReferencingOptions: Bool,
        title: String,
        referencePassword: Bool,
        referenceUsername: Bool,
        preserveTimestamps: Bool,
        editAfterwards: Bool,
        completion: ((Bool, String, Bool, Bool, Bool, Bool) -> Void)?
    ) -> NSViewController {
        NSHostingController(rootView: DuplicateItemOptionsView(showReferencingOptions: showReferencingOptions,
                                                               title: title,
